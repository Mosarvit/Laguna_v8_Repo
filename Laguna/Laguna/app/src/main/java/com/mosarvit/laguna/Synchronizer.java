...
public class Synchronizer<T extends OnServerModel> {
    ...
    private void initializeAllMemberVariables() {
        ...
        allFcsHere = new HashMap<>();
        newFcsHere = new HashMap<>();
        notNewFcsHere = new HashMap<>();
        fcsServer = new HashMap<>();

        toInsertHere = new ArrayList<>();

        toRequestFromServer = new ArrayList<>();
        toUpdateOnServer = new ArrayList<>();
        toInsertOnServer = new ArrayList<>();

        toDeleteHere = new ArrayList<>();
        toDeleteOnServer = new ArrayList<>();

        updatedOnlyHereAfterLoading = new ArrayList<>();
        updatedOnlyOnServerAfterLoading = new ArrayList<>();

        contradictingHereVersion = new ArrayList<>();
        contradictingServerVersion = new ArrayList<>();
    ...
    private void onVolleyError(VolleyError volleyError) {
        ...
        if (volleyError.networkResponse == null) {
            if (volleyError.getClass().equals(TimeoutError.class)) {
                ...
                if (timeOutsSoFar < SharedClass.MAX_RETRIES_ON_TIMEOUT) {
                    printLine("TIME OUT");
                    synchronize();
                    return;
                } else {
...
//                printLine("response: " + response);
                ...
                try {
                    ...
                    if (JA.length() != toRequestFromServer.size()) {
                        badResponse("Too little Flashcards arrived");
                    }
                ...
                if (modelType == MediaFileSegment.class) {
                    UpdateUpDownloadFlags();
                }
...
//                        );
    ...
    private void UpdateUpDownloadFlags() {

//        printLine("updating flags");
//        printLine("toInsertHere: " + toInsertHere.size());

        for (MediaFileSegment mfs : (List<MediaFileSegment>) toInsertHere) {

            mfs.setToDownload(true);
            mfs.save();
//            printLine("mfs.getToDownload(): " + mfs.getToDownload());
        }

        for (MediaFileSegment mfs : (List<MediaFileSegment>) toUpdateOnServer) {

            mfs.setToUpload(true);
            mfs.save();
        }

        for (MediaFileSegment mfs : (List<MediaFileSegment>) toInsertOnServer) {
...
//        printLine("response:" + response);
        ...
        if (response.equals("") || response.equals(" ")  || response.equals("null")) {
            return;
        }
    ...
    private void UploadToServer() throws IOException {
        ...
        for (Model m : new Select().from(MediaFileSegment.class).where("toUpload = 1").execute()) {

            MediaFileSegment mfs = (MediaFileSegment) m;
            new UploadToFtpTask(mfs, view).execute();
...
//        printLine("total to Download : " + new Select().from(MediaFileSegment.class).where("toDownload = 1").count());

        for (Model m : new Select().from(MediaFileSegment.class).where("toDownload = 1").execute()) {

            MediaFileSegment mfs = (MediaFileSegment) m;

            String ftpFilePath = ftpMediaFolder + "/" + mfs.mediaFileName + "/" + mfs.fileName;

            printLine("To Download: " + FileHelper.getName(ftpFilePath));
            ...
            new DownloadMfsTask(mfs, view).execute();
    ...
    private void finishSync() {
        ...
        printLine("Number of MediaFileSegment in Db: " + new Select().from(MediaFileSegment.class).count());

        view.setFinishOnTouchOutside(true);
        SharedClass.setLastSyncSuccessful(success, view);
        if (success) {
    ...
    public void nextTask() {
        ...
        if (taskCount == 1) {
            ...
            if (success && modelType == Flashcard.class) {

                printLine("Number of Flashcards in Db: " + new Select().from(Flashcard.class).count());
                Synchronizer<MediaFileSegment> sM = new Synchronizer<>(view, MediaFileSegment.class);
                sM.synchronize();
            } else {
                try {
